...
if [[ ! -d "0" ]]; then
  echo "Directory '0' does not exist. Creating it..."
  mkdir -p "0"
else
  echo "Directory '0' already exists."
  (cd 0 && rm -f -- *)
fi

# Source the bashrc file
source "$WM_PROJECT_USER_DIR/applications/utilities/waves2Foam/bin/bashrc" noPrint
...
if [[ $WM_PROJECT_VERSION_NUMBER -lt 170 ||  $FOAMEXTENDPROJECT -eq 1 ]]; then
  sed 's/object      pd.org;/object      pd;/' < 0.org/pd.org > 0/pd
else
  if [[ "$OFPLUSBRANCH" -eq 1 ]]; then
    if [[ $WM_PROJECT_VERSION_NUMBER -lt 1706 ]]; then
      sed 's/object      pd.org;/object      p_rgh;/' < 0.org/pd.org > 0/p_rgh
    else
      cp 0.org/p_rgh.1706.org 0/p_rgh
    fi
  else
    if [[ $WM_PROJECT_VERSION_NUMBER -lt 400 ]]; then
      sed 's/object      pd.org;/object      p_rgh;/' < 0.org/pd.org > 0/p_rgh
    else
